...
function weatherPage(weatherData) {
  const temperature = weatherData.temperatureC === null ? 'Unavailable' : `${escapeHtml(weatherData.temperatureC)}°C`;
  const source = weatherData.source === 'live' ? 'Live weather' : weatherData.source === 'cache' ? 'Recently cached weather' : weatherData.source === 'stale-cache' ? 'Previously cached weather' : 'Weather unavailable';
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Weather | Papertrail Books</title><link rel="stylesheet" href="/styles.css"></head><body><header><a class="brand" href="/">Papertrail <i>Books</i></a><nav><a href="/">Home</a></nav></header><main><section class="weather" aria-label="Current weather"><p class="eyebrow">${source}</p><h1>${escapeHtml(weatherData.location)} weather</h1><p>${escapeHtml(weatherData.condition)} · ${temperature}</p><small>Updates at most once every 15 minutes.</small></section></main></body></html>`;
}
...
function createApp({ weatherService = weather } = {}) {
  return http.createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && url.pathname === '/weather') {
      const conditions = await weatherService.getConditions();
      response.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return response.end(weatherPage(conditions));
    }
